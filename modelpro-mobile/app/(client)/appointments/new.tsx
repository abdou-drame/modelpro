...
function ArtisanPicker({
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      artisansApi.search({ atelier: debouncedSearch || undefined, limit: 20 }).then((r) => r.data ?? []),
  ...
  return (
    <View style={picker.wrapper}>
      ...
      {showDropdown && (
        <View style={picker.dropdown}>
          ...
          {!isLoading && results.length === 0 && (
            <View style={picker.dropdownState}>
              <Text style={picker.dropdownStateText}>Aucun résultat pour "{debouncedSearch}"</Text>
            </View>
          )}
          {results.map((a, i) => (
            <TouchableOpacity
      ...
      {!showDropdown && debouncedSearch.length < 2 && (
        <Text style={picker.hint}>Tapez au moins 2 caractères</Text>
      )}
    </View>
  )
}
...
export default function NewAppointmentScreen() {
  ...
  const { control, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormValues>({
    ...
    defaultValues: { heure: '10:00', date: '', lieu: '', notes: '' },
  })
    ...
    >
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          {!preloadedArtisanId && (
            <Animated.View entering={FadeInUp.delay(40).springify()} style={styles.section}>
          ...
          {preloadedArtisan && (
            <Animated.View entering={FadeInUp.delay(40).springify()} style={styles.artisanContext}>
              <Image
          ...
          {/* ── Date and time ── */}
          <Animated.View entering={FadeInUp.delay(140).springify()} style={styles.dateTimeRow}>
          ...
          <Animated.View entering={FadeInUp.delay(300).springify()}>
            <TouchableOpacity
              style={[
                styles.submitBtn,
                (!artisanReady || isSubmitting || mutation.isPending) && styles.submitBtnDisabled,
              ]}
              onPress={handleSubmit(onSubmit)}
              disabled={!artisanReady || isSubmitting || mutation.isPending}
              accessibilityRole="button"
