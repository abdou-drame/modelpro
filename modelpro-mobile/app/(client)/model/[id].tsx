import {
  View, Text, Image, TouchableOpacity, ScrollView, StyleSheet, Dimensions, Share, Platform,
} from 'react-native'
...
const HERO_HEIGHT = Math.min(Math.round(width * 1.1), 480)
...
export default function ModelDetailScreen() {
  ...
  const handleShare = async () => {
    try {
      await Share.share({ message: `${model.titre} — ${atelier}` })
    } catch {}
  }
      ...
      <ScrollView showsVerticalScrollIndicator={false} bounces>
        ...
        <View style={styles.body}>
          ...
          <Animated.View entering={FadeInUp.delay(80).springify()} style={styles.titleBlock}>
            <Badge label={metier} variant="primary" size="sm" />
          ...
          {model.artisan && (
            <Animated.View entering={FadeInUp.delay(130).springify()}>
              <TouchableOpacity
                ...
                onPress={() => model.artisan && router.push(`/(client)/artisan/${model.artisan.id}`)}
                activeOpacity={0.85}
                accessibilityRole="button"
                accessibilityLabel={`Voir le profil de ${atelier}`}
              >
                {/* Avatar placeholder ring */}
                <View style={styles.artisanAvatar}>
                  <Text style={styles.artisanAvatarText}>
                    {atelier[0]?.toUpperCase() ?? 'A'}
                  </Text>
                </View>
