...
export default function Users() {
  ...
  const paginationMeta = data ? { page: (data as any).page ?? 1, totalPages: (data as any).totalPages ?? 1, total: (data as any).total ?? users.length, limit: (data as any).limit ?? 10 } : null
  ...
  return (
    <div>
      ...
      <ConfirmDialog
        ...
        description={`${pendingDelete?.prenom} ${pendingDelete?.nom} et toutes ses données seront définitivement supprimés de la plateforme. Cette action est irréversible.`}
