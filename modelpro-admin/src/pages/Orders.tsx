...
export default function Orders() {
  ...
  const allOrders = Array.isArray(allData) ? allData : (allData?.data ?? [])
  const totalOrdersCount = Array.isArray(allData) ? ((allData as any).total ?? allOrders.length) : (allData?.total ?? allOrders.length)
  const paginationMeta = tab === 'all' && allData
    ? { page: (allData as any).page ?? 1, totalPages: (allData as any).totalPages ?? 1, total: totalOrdersCount, limit: (allData as any).limit ?? 10 }
    : null
  ...
  const filtered = useMemo(() => {
    ...
    return baseOrders.filter((o) => {
      const clientName = `${o.client?.prenom ?? ''} ${o.client?.nom ?? ''}`.toLowerCase()
      const atelierName = (o.artisan?.atelier ?? '').toLowerCase()
      const matchSearch =
        !q ||
        clientName.includes(q) ||
        atelierName.includes(q)
                ...
                filtered.map((order) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-ink font-medium whitespace-nowrap">
                      {order.client ? `${order.client.prenom ?? ''} ${order.client.nom ?? ''}`.trim() || 'Client' : 'Client'}
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-ink font-medium">{order.artisan?.atelier ?? 'Atelier'}</span>
