...
export default function Artisans() {
  ...
  const { data: artisansData, isLoading: loadingAll } = useQuery({
    queryKey: ['admin-artisans', page, search],
    queryFn: () => artisansAdminApi.list({ page, limit: 10, search: search || undefined } as any).then((r) => r.data),
    placeholderData: (prev) => prev,
    enabled: activeTab === 'all',
  })
  const artisans = Array.isArray(artisansData) ? artisansData : (artisansData?.data ?? [])
  const artisansMeta = artisansData ? { page: (artisansData as any).page ?? 1, totalPages: (artisansData as any).totalPages ?? 1, total: (artisansData as any).total ?? artisans.length, limit: (artisansData as any).limit ?? 10 } : null
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-6">
        ...
        {activeTab === 'all' && (
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              placeholder="Rechercher par nom, atelier, métier..."
              value={search}
